...
interface DiskWriteProps {
  starttime: Date;
  endtime: Date;
}

interface DiskWriteApiResponseMetric {
  device: string;
  instance: string;
  job: string;
}
...
const DiskWrite: React.FC<DiskWriteProps> = ({starttime, endtime}) => {
  ...
  useEffect(() => {
    const fetchDiskWriteData = async () => {
      const query = `rate(node_disk_writes_completed_total[30s])`;
      const response
        = await invokeQueryRange<QueryRangeResponse<DiskWriteApiResponseMetric>>(query, starttime, endtime, prometheusSettings?.postgresqlScrapeInterval);
        ...
        <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', marginBottom: '-15px', marginTop: '-5px', width: '100%' }}>
          ...
          >
            <Typography sx={{ p: 2 , alignItems: 'center'}} style={{ width: '600px', whiteSpace: 'pre-line' }}>
              ...
              秒毎のディスク書き込み操作の平均完了回数をカウントします。
              ディスクの書き込み操作の数が急激に増加すると、システムに高いI/O負荷がかかっている可能性があり、パフォーマンスの問題やボトルネックの原因となり得ます。
