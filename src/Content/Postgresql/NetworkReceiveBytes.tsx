...
interface ReceiveBytesProps {
  starttime: Date;
  endtime: Date;
}

interface ReceiveBytesApiResponseMetric {
  device: string;
  instance: string;
  job: string;
}
...
const NetworkReceiveBytes: React.FC<ReceiveBytesProps> = ({starttime, endtime}) => {
  ...
  useEffect(() => {
    const fetchDiskReadData = async () => {
      const query = `rate(node_network_receive_bytes_total[30s])`;
      const response
        = await invokeQueryRange<QueryRangeResponse<ReceiveBytesApiResponseMetric>>(query, starttime, endtime, prometheusSettings?.postgresqlScrapeInterval);
        ...
        <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', marginBottom: '-15px', marginTop: '-5px', width: '100%' }}>
          ...
          >
            <Typography sx={{ p: 2 , alignItems: 'center'}} style={{ width: '600px', whiteSpace: 'pre-line' }}>
              ...
              秒ごとに受信するバイト数をカウントします。
              カウンター値が増加すると、それはそのノードがより多くのデータをネットワークから受信していることを意味します。
              予想外の高い受信バイト数は、ネットワークのパフォーマンス問題やセキュリティ上の脅威を示唆している可能性があります。
            </Typography>
          </Popover>
...
export default NetworkReceiveBytes;
